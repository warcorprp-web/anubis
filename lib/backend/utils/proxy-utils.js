...
// Global config cache for hot reload
let globalConfigCache = null;

export function setGlobalProxyConfig(config) {
  globalConfigCache = config;
  logger.debug('[Proxy] Global config updated');
}
...
export function getProxyConfigForProvider(config, providerType) {
  // Use global config cache if available (for hot reload)
  const actualConfig = globalConfigCache || config;
  
  if (!isProxyEnabledForProvider(actualConfig, providerType)) {
    logger.debug(`[Proxy] Proxy disabled for ${providerType}`);
    return null;
  }
  const proxyConfig = parseProxyUrl(actualConfig.PROXY_URL);
  if (proxyConfig) {
    logger.info(`[Proxy] ✓ Using ${proxyConfig.proxyType.toUpperCase()} proxy for ${providerType}: ${actualConfig.PROXY_URL}`);
  } else {
    logger.warn(`[Proxy] Failed to configure proxy for ${providerType}: invalid URL ${actualConfig.PROXY_URL}`);
  }
  return proxyConfig;
}
...
export function configureAxiosProxy(axiosConfig, config, providerType) {
  ...
  logger.debug(`[Proxy] configureAxiosProxy called for ${providerType}, cache: ${!!globalConfigCache}, enabled: ${actualConfig?.PROXY_ENABLED_PROVIDERS?.includes(providerType)}`);
  
  const proxyConfig = getProxyConfigForProvider(actualConfig, providerType);
  if (proxyConfig) {
