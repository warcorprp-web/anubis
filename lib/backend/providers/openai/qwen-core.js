...
export class QwenApiService {
  constructor(config) {
    ...
    logger.info(`[Qwen] Constructor config: ${JSON.stringify({
      QWEN_BASE_URL: config.QWEN_BASE_URL,
      QWEN_OAUTH_BASE_URL: config.QWEN_OAUTH_BASE_URL,
      QWEN_OAUTH_CREDS_FILE_PATH: config.QWEN_OAUTH_CREDS_FILE_PATH,
      uuid: config.uuid,
  ...
  async getValidToken(forceRefresh = false) {
    try {
      ...
      const endpoint = this.getCurrentEndpoint(credentials.resource_url);
      logger.info(`[Qwen] getValidToken: forceRefresh=${forceRefresh}, resource_url=${credentials.resource_url}, endpoint=${endpoint}, token=${credentials.access_token.substring(0, 20)}...`);
      return {
  ...
  async callApiWithAuthAndRetry(endpoint, body, isStream = false, retryCount = 0) {
    ...
    try {
      const forceRefresh = this.firstRequest || retryCount > 0;
      if (this.firstRequest) {
        logger.info(`[Qwen] First request - forcing token refresh`);
        this.firstRequest = false;
      }
      const {token: token, endpoint: qwenBaseUrl} = await this.getValidToken(forceRefresh);
      const httpAgent = new http.Agent({
        keepAlive: true,
        maxSockets: 100,
        maxFreeSockets: 5,
      ...
      logger.info(`[QwenApiService] Full URL: ${qwenBaseUrl}${endpoint}`);
      logger.info(`[QwenApiService] Body: ${JSON.stringify(requestBody)}`);
      logger.info(`[QwenApiService] Headers: ${JSON.stringify(axiosConfig.headers)}`);
  ...
  async performTokenRefresh(context, qwenClient, forceRefresh = false) {
    ...
    try {
      ...
      try {
        await this.checkAndReloadIfNeeded(context);
        if (!forceRefresh && context.memoryCache.credentials && this.isTokenValid(context.memoryCache.credentials)) {
          logger.info(`[Qwen] performTokenRefresh: Token valid, returning cached (forceRefresh=${forceRefresh})`);
          qwenClient.setCredentials(context.memoryCache.credentials);
          return context.memoryCache.credentials;
        }
        logger.info(`[Qwen] performTokenRefresh: Calling refreshAccessToken (forceRefresh=${forceRefresh})`);
