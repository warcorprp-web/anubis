...
class ResponsesAPIStrategy extends ProviderStrategy {
  ...
  async applySystemPromptFromFile(config, requestBody) {
    ...
    if (!requestBody.instructions || config.SYSTEM_PROMPT_MODE === "append") {
      if (typeof requestBody.input === "string") {
        ...
        const systemMessageIndex = requestBody.input.findIndex(m => m.role === "system" || m.type && m.type === "system");
  ...
  async manageSystemPrompt(requestBody) {
    ...
    if (requestBody.instructions) {
      ...
      const systemMessage = requestBody.input.find(item => item.role === "system" || item.type && item.type === "system");
