...
import { DeepSeekProvider } from "./deepseek/deepseek-core.js";
...
export class DeepSeekApiServiceAdapter extends ApiServiceAdapter {
  constructor(config) {
    super();
    this.deepseekProvider = null;
    this.config = config;
    this.initPromise = this._init();
  }
  async _init() {
    this.deepseekProvider = await new DeepSeekProvider({
      ...this.config,
      authToken: this.config.DEEPSEEK_AUTH_TOKEN
    }).init();
  }
  async generateContent(model, requestBody) {
    await this.initPromise;
    logger.info(`[DeepSeek Adapter] generateContent called, messages:`, requestBody.messages);
    const messages = requestBody.messages || [];
    let fullContent = '';
    let chunkCount = 0;
    for await (const chunk of this.deepseekProvider.chatCompletion(messages, {})) {
      chunkCount++;
      logger.debug(`[DeepSeek Adapter] Chunk ${chunkCount}:`, chunk);
      if (chunk.choices?.[0]?.delta?.content) {
        fullContent += chunk.choices[0].delta.content;
      }
    }
    logger.info(`[DeepSeek Adapter] Generated content (${chunkCount} chunks): ${fullContent.substring(0, 100)}...`);
    return { choices: [{ message: { content: fullContent } }] };
  }
  async* generateContentStream(model, requestBody) {
    await this.initPromise;
    const messages = requestBody.messages || [];
    yield* this.deepseekProvider.chatCompletion(messages, {});
  }
  async listModels() {
    return [{ id: 'deepseek-chat' }, { id: 'deepseek-coder' }];
  }
  async refreshToken() {
    return Promise.resolve();
  }
...
registerAdapter(MODEL_PROVIDER.DEEPSEEK, DeepSeekApiServiceAdapter);
