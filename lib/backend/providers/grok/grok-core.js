...
const CHROME_CIPHERS = [ "TLS_AES_128_GCM_SHA256", "TLS_AES_256_GCM_SHA384", "TLS_CHACHA20_POLY1305_SHA256", "ECDHE-ECDSA-AES128-GCM-SHA256", "ECDHE-RSA-AES128-GCM-SHA256", "ECDHE-ECDSA-AES256-GCM-SHA384", "ECDHE-RSA-AES256-GCM-SHA384", "ECDHE-ECDSA-CHACHA20-POLY1305", "ECDHE-RSA-CHACHA20-POLY1305", "ECDHE-RSA-AES128-SHA", "ECDHE-RSA-AES256-SHA", "AES128-GCM-SHA256", "AES256-GCM-SHA384", "AES128-SHA", "AES256-SHA" ].join(":");

const CHROME_SIGALGS = [ "ecdsa_secp256r1_sha256", "rsa_pss_rsae_sha256", "rsa_pkcs1_sha256", "ecdsa_secp384r1_sha384", "rsa_pss_rsae_sha384", "rsa_pkcs1_sha384", "rsa_pss_rsae_sha512", "rsa_pkcs1_sha512" ].join(":");
...
export class GrokApiService {
  ...
  async acceptTos() {
    const axiosConfig = {
      method: "post",
      url: `${this.baseUrl}/rest/app-chat/accept-tos`,
  ...
  async enableNsfwAccount() {
    ...
    const inner = Buffer.concat([ Buffer.from([ 10, name.length ]), name ]);
    const protobuf = Buffer.concat([ Buffer.from([ 10, 2, 16, 1, 18, inner.length ]), inner ]);
  ...
  isExpiryDateNear() {
    ...
    return Date.now() - this.lastSyncAt > (this.config.CRON_NEAR_MINUTES || 15) * 60 * 1e3;
  }
  genStatsigId() {
    ...
    const msg = Math.random() < .5 ? `e:TypeError: Cannot read properties of null (reading 'children['${randomString(5, true)}']')` : `e:TypeError: Cannot read properties of undefined (reading '${randomString(10)}')`;
  ...
  _extractPostId(text) {
    ...
    const match = text.match(/\/post\/([0-9a-fA-F-]{32,36})/) || text.match(/\/generated\/([0-9a-fA-F-]{32,36})\//) || text.match(/\/([0-9a-fA-F-]{32,36})\/generated_video/);
  ...
  buildPayload(modelId, requestBody) {
      ...
      for (const msg of processedMessages) {
        ...
        if (typeof content === "string") {
          ...
          for (const item of content) {
            if (item.type === "text" && item.text?.trim()) parts.push(item.text.trim()); else if (item.type === "image_url" && item.image_url?.url) imageAttachments.push(item.image_url.url); else if (item.type === "file" && item.file?.file_data) localFileAttachments.push(item.file.file_data);
          }
        }
        ...
        if (parts.length > 0) extracted.push({
          role: role,
          text: parts.join("\n")
        });
      }
    ...
    const payload = {
      ...
      disableMemory: false,
      disableNsfwFilter: isNsfw,
      disableSearch: false,
      disableSelfHarmShortCircuit: false,
      disableTextFollowUps: false,
      enableImageGeneration: isMediaModel,
      enableImageStreaming: isMediaModel,
  ...
  async* generateContentStream(model, requestBody, retryCount = 0) {
    ...
    if (this.isExpiryDateNear() && getProviderPoolManager() && this.uuid) {
      getProviderPoolManager().markProviderNeedRefresh(MODEL_PROVIDER.GROK_CUSTOM, {
        uuid: this.uuid
      });
    }
    ...
    if (isVideoModel) {
      ...
      if (!parentPostId) {
        ...
        if (requestBody.messages?.length > 0) {
          ...
          if (lastMsg.role === "user" && Array.isArray(lastMsg.content)) {
            lastMsg.content.forEach(item => {
              if (item.type === "image_url" && item.image_url?.url) lastMsgImages.push(item.image_url.url);
            });
          }
        }
      ...
      if (parentPostId) {
        requestBody.videoGenModelConfig = {
    ...
    try {
      ...
      const {status: status, errorCode: errorCode, errorMessage: errorMessage, isNetworkError: isNetworkError} = this.classifyApiError(error);
  ...
  async listModels() {
    return {
      data: GROK_MODELS.map(id => ({
        ...
        owned_by: "xai",
        display_name: id.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")
      }))
    };
  }
}